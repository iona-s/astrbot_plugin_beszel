...
- **单机详情 (`/beszel status`)**：单探针完整实时详情长图，展示系统规格、运行时间、多核心负载、网络/磁盘/GPU 明细占用、外挂磁盘（EFS）和 Docker 容器实时资源占用列表。
- **历史监控 (`/beszel history`)**：双列历史时序长图，支持多指标聚合卡片（系统 CPU/内存/磁盘、网络 Rx/Tx、磁盘读写 I/O、1m/5m/15m 负载、GPU 功耗与显存、外挂盘独立 I/O、以及 Docker/Podman 容器 CPU 与内存历史趋势折线图等）。
...
> 💡 **时间跨度支持**：`1h`（1小时）、`12h`（12小时）、`24h`（24小时）、`1w`（1周）、`30d`（30天）。若探针包含 Docker/Podman 容器且存在时序记录，将自动在主机图后呈现容器 CPU 与内存趋势折线图（默认过滤峰值低于 Y 轴最大值 10% 的容器以消除微量噪声，超出/被过滤数量展示 `+N` 标识，阈值可配置）。
...
- **`container_history_threshold`**：容器历史过滤阈值（默认 `10`%，可调范围 `0 ~ 50`，设为 `0` 则全显示；建议设为 `5 ~ 20`）
- **`render_scale`**：图片渲染精细度（默认 `100`%，可调范围 `50 ~ 300`，调高可提升清晰度但增加内存开销，调低则反之）
